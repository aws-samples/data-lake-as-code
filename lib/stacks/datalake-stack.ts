...
export class DataLakeStack extends cdk.Stack {
  ...
  constructor(scope: cdk.Construct, id: string, props: DataLakeStackProps) {
            ...
            "Statement": [
                {
                    ...
                    "Resource": [
                        this.AthenaResultsBucket.bucketArn, this.AthenaResultsBucket.bucketArn+"/*"
                    ]
                }
            ]
    };
        
    const coarseAthenaResultBucketAccessPolicyDoc = iam.PolicyDocument.fromJson(coarseAthenaResultBucketAccess);
    ...
    workGroupConfigCustResourceRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSLambdaBasicExecutionRole'));

    workGroupConfigCustResourceRole.addToPolicy(new iam.PolicyStatement({
    ...
    const workGroupConfigCustResource = new cfn.CustomResource(this, 'workGroupConfigCustResource', {
        provider: cfn.CustomResourceProvider.lambda(new lambda.SingletonFunction(this, 'Singleton', {
            ...
            code: new lambda.InlineCode(fs.readFileSync('scripts/lambda.updateprimaryworkgroup.py', { encoding: 'utf-8' })),
            handler: 'index.main',
